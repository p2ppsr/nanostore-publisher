...
describe('pay function', () => {
  ...
  it('should create a payment using Ninja when clientPrivateKey is provided', async () => {
    ...
    (Ninja as jest.Mock).mockImplementation(() => ({
      getTransactionWithOutputs: jest.fn().mockResolvedValue(mockNinjaTransaction)
    }));
  ...
  it('should create a payment using createAction when clientPrivateKey is not provided', async () => {
    const configWithoutPrivateKey = { ...mockConfig, clientPrivateKey: undefined };
    ...
    const result = await pay({ ...mockPayParams, config: configWithoutPrivateKey });
  ...
  it('should throw an error if createAction returns an error status', async () => {
    ...
    await expect(pay({ ...mockPayParams, config: configWithoutPrivateKey }))
  ...
  it('should throw an error if AuthriteClient returns an error status', async () => {
    ...
    await expect(pay({ ...mockPayParams, config: mockConfig }))
  ...
  it('should use default CONFIG if no config is provided', async () => {
    ...
    expect(derivePaymentInfo).toHaveBeenCalledWith(expect.objectContaining({
      config: CONFIG,
  ...
  it('should handle input validation', async () => {
    await expect(pay({ ...mockPayParams, amount: -1 })).rejects.toThrow('Invalid amount');
    await expect(pay({ ...mockPayParams, orderID: '' })).rejects.toThrow('Invalid order ID');
    await expect(pay({ ...mockPayParams, recipientPublicKey: '' })).rejects.toThrow('Invalid recipient public key');
  });
});
