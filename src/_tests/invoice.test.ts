...
describe('invoice function', () => {
  ...
  it('should use default CONFIG if no config is provided', async () => {
    ...
    expect(AuthriteClient).toHaveBeenCalledWith(
      CONFIG.nanostoreURL,
      CONFIG.clientPrivateKey ? { clientPrivateKey: CONFIG.clientPrivateKey } : undefined
    );
  });
  ...
  it('should create an invoice with maximum allowed inputs', async () => {
    ...
    const result = await invoice({ fileSize: 1000000000, retentionPeriod: 525600 }); // 1GB, 1 year
    expect(result).toEqual(mockInvoiceResponse);
  });
  it('should throw an error for invalid inputs', async () => {
    await expect(invoice({ fileSize: -1, retentionPeriod: 60 })).rejects.toThrow('Invalid file size');
    await expect(invoice({ fileSize: 1024, retentionPeriod: -1 })).rejects.toThrow('Invalid retention period');
    await expect(invoice({ fileSize: 'invalid' as any, retentionPeriod: 60 })).rejects.toThrow();
    await expect(invoice({ fileSize: 1024, retentionPeriod: 'invalid' as any })).rejects.toThrow();
  });
  ...
  it('should use custom config when provided', async () => {
    ...
    await invoice({ config: customConfig, fileSize: 1024, retentionPeriod: 60 });
    expect(AuthriteClient).toHaveBeenCalledWith(
  ...
  it('should handle different error responses correctly', async () => {
    const errorResponses = [
      { status: 'error', description: 'Invalid file size', code: 'INVALID_SIZE' },
      { status: 'error', description: 'Invalid retention period', code: 'INVALID_PERIOD' },
    ...
    for (const errorResponse of errorResponses) {
      ...
      await expect(invoice({ fileSize: 1024, retentionPeriod: 60 }))
        .rejects.toThrow(errorResponse.description);
  
      const thrownError = await invoice({ fileSize: 1024, retentionPeriod: 60 }).catch(e => e);
  ...
  it('should initialize AuthriteClient without clientPrivateKey when not provided', async () => {
    ...
    await invoice({ config: configWithoutPrivateKey, fileSize: 1024, retentionPeriod: 60 });
    expect(AuthriteClient).toHaveBeenCalledWith(
