...
describe('publishFile function', () => {
  ...
  it('should throw an error if file is missing', async () => {
    await expect(publishFile({ config: mockConfig, retentionPeriod: 60 } as any)).rejects.toThrow('Choose a file to upload!');
  });

  it('should throw an error if retentionPeriod is missing', async () => {
    await expect(publishFile({ config: mockConfig, file: mockFile } as any)).rejects.toThrow('Specify how long to host the file!');
  });

  it('should use default CONFIG if no config is provided', async () => {
    await publishFile({ file: mockFile, retentionPeriod: 60 });
    expect(invoice).toHaveBeenCalledWith(expect.objectContaining({ config: CONFIG }));
  });
