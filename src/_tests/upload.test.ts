...
describe('upload function', () => {
  ...
  it('should upload file to local storage', async () => {
    const mockFile = new File([Buffer.from('test content')], 'test.txt', { type: 'text/plain' });
    const result = await upload({
  ...
  it('should handle different file types correctly', async () => {
    ...
    await upload({ file: textFile as any, uploadURL: 'https://example.com/upload', publicURL: 'https://example.com/public' });
    expect(axios.put).toHaveBeenCalledWith(
      expect.any(String),
      expect.any(Blob),
    ...
    await upload({ file: imageFile as any, uploadURL: 'https://example.com/upload', publicURL: 'https://example.com/public' });
    expect(axios.put).toHaveBeenCalledWith(
      expect.any(String),
      expect.any(Blob),
  ...
  it('should handle CustomFile with dataAsBuffer for local upload', async () => {
    const mockAxios = jest.spyOn(axios, 'post').mockResolvedValue({ data: 'hash' });
  ...
  it('should handle errors for local server uploads', async () => {
    (axios.post as jest.Mock).mockRejectedValueOnce(new Error('Local server error'));
